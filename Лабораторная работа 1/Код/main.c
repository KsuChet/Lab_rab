...
//gaussing window
void gaussing(float* inputData, float* outputData, int n) {
	int i = 0;
	for(i = 0; i < n; i++) {
		if(i < N) {
			outputData[i] = exp(-12.5 * pow((i - (N - 1) / 2) / (N - 1), 2));
		} else {
			outputData[i] = 0;
		}
		outputData[i] *= inputData[i];
...
//barlett window
void barlett(float* inputData, float* outputData, int n) {
	int i = 0;
	for(i = 0; i < n; i++) {
		outputData[i] = 0;
		if(i < N) {
			 outputData[i] = 1 - 2 * abs(i - (N - 1) / 2) / (N - 1);
		}
		outputData[i] *= inputData[i];
	}
}
...
void main() {
	...
	//gaussian window
	gaussing(inBuffer, outBuffer, MAX_SIZE);
	//spectr of signal with window
	for(k = 0; k < MAX_SIZE; k++) {
		spectrSignal = dft(outBuffer, MAX_SIZE, k);
		arrToShow[k] = curve(spectrSignal);
	...
	//barlett window
	barlett(inBuffer, outBuffer, MAX_SIZE);
	//spectr of signal with window
	for(k = 0; k < MAX_SIZE; k++) {
		spectrSignal = dft(outBuffer, MAX_SIZE, k);
		arrToShow[k] = curve(spectrSignal);
